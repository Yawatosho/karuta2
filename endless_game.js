...
function fetchAndRenderRanking(targetId, period='day', ym=getTodayYMDJST(), reqId=null, options = {}) {
  ...
  let url = buildRankingURLByPeriod(period, ym, limit, ENDLESS_RANKING_PARAMS);
  if (forceNoCache) {
...
function startGame() {
  ...
  Promise.all([fetchCards(), audioReady]).then(([fetchedCards]) => {
...
function setupWave(fetchedCards) {
  cards = fetchedCards;
...
function nextRound() {
  ...
  if (round >= WAVE_ROUNDS) { completeWave(); return; }
  hideCountdown();
...
function selectCard(e) {
  ...
  if (selectedCard.ndc === currentReadingCard.ndc) {
    ...
    setMessage('', '', '');
    setTimeout(() => {
      setMessage('warning', lives <= 0 ? 'LIFE OUT' : 'MISS', lives <= 0 ? 'GAME OVER' : `SCORE -500pt / LIFE ${lives}/${MAX_LIVES}`);
    }, 200);
    ...
    if (lives <= 0) {
      ...
      clearInterval(timeDisplayInterval);
      clearTimeout(roundResultTimeout);
      roundResultTimeout = setTimeout(endGame, 1300);
    } else {
...
// ===== ローカルハイスコア =====
const LOCAL_HIGH_SCORES_KEY = 'localHighScoresEndless';
...
function displayUsedCards() {
  ...
  let html = '<h2 class=resultheading>LAST WAVE CARDS</h2><ul>';
...
function renderResultScoreSummary() {
  if (!resultScoreSummaryEl) return;
  const note = `WAVE ${wave} / LIFE ${lives}/${MAX_LIVES}`;
